...
public class LdapAuthenticationProvider implements AuthenticationProvider {
		...
		if (!StringUtils.isEmpty(pwr.getOldPassword())) {
			...
			try {
				...
				auditLogger.log("LDAP_AUTH", "PW_RESET", "", "", null,
						"Password reset with old password failed for: " + pwr.getUsername());
				return false;
			}
			...
			auditLogger.log("LDAP_AUTH", "PW_RESET", "", "", null,
					"Password reset with token failed for: " + pwr.getUsername() + " (incorrect token)");
			return false;
		} else {
			if (lu == null) {
			...
			auditLogger.log("LDAP_AUTH", "PW_RESET", "", "", null,
					"Password reset initiated for: " + pwr.getUsername());
			...
			params.put("user", lu);
			try {
				mailServiceFactory.getDefaultMailService().sendMail(lu.getMail(), "ldapconfig", "passwordReset", null,
						params);
			} catch (Exception e) {
				throw new AuthenticationFailedException("error sending mail", e);
			}
			return true;
		}
	}
	...
	public List<User> getLinked(User u) throws Exception {
		...
		for (LdapConfig lConf : ldapConfigService.list(null, null, null)) {
			if (lConf.getId().equals(u.getSourceId())) {
				out.add(u);
			} else {
