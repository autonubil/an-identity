...
public abstract class AbstractLdapConnection implements LdapConnection {
	...
	public NamingEnumeration<SearchResult> list(String base, String filter, String[] attributes) throws NamingException {
		SearchControls searchControls = new SearchControls();
		searchControls.setSearchScope(SearchControls.SUBTREE_SCOPE);
	    searchControls.setReturningAttributes(attributes);
	    String dn = LdapEncoder.escapeDn(base);
	...
	public <T> List<T> list(String base, String filter, LdapSearchResultMapper<T> mapper, String[] attributes) throws NamingException {
	    List<T> out = new ArrayList<>();
	...
	protected ModificationItem createItem(SearchResult r, String attributeName, Object newAttributeValue) {
		...
		if(oldValue==null) {
			if(newAttributeValue==null || StringUtils.isEmpty(newAttributeValue.toString())) {
				...
				return new ModificationItem(DirContext.ADD_ATTRIBUTE, new BasicAttribute(attributeName,newAttributeValue));				
			}
		} else {
			if(newAttributeValue==null || StringUtils.isEmpty(newAttributeValue.toString())) {
				...
				return new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute(attributeName,newAttributeValue));
			}
		}
	}
	...
	public void createEntry(String dn, String[] classes, Map<String,Object> attributes) throws NamingException {
		...
		try {
			...
			AuditLoggerHelper.log("LDAP", "CREATE", "", user, config.getName()+":"+dn, "LDAP UPDATE: "+new ObjectMapper().enable(SerializationFeature.INDENT_OUTPUT).writeValueAsString(changes));
		} catch (Exception e) {
			log.error("error writing audit log",e);
			throw new NamingException("failed to write audit log");
		}
	...
	public void updateUserAttribute(String dn, String attribute, Object attributeValue, boolean replace) throws NamingException {
		if(attributeValue instanceof Date) {
			attributeValue = formatDate((Date)attributeValue); 
		}
		ModificationItem[] mi =
			new ModificationItem[] {
				new  ModificationItem(replace?DirContext.REPLACE_ATTRIBUTE:DirContext.ADD_ATTRIBUTE, new BasicAttribute(attribute,attributeValue))
		};
		modifyEntry(dn, mi);
	}
	...
	public List<LdapGroup> getGroupsForUser(String userId, boolean recursive) throws Exception {
		...
		if(recursive) {
			for(LdapGroup g : new ArrayList<>(groups)) {
				getGroupsForGroup(groups,g,recursive);
			}
		}
	...
	public void removeUserFromGroup(String userId, String groupId) throws Exception {
		...
		SearchResult sr = get(getGroupSearchBase(),getGroupGetFilter(groupId),new String[] {"member"});
		if(sr == null) return;
		if(sr.getAttributes() == null) return;
		if(sr.getAttributes().get("member") == null) return;
		NamingEnumeration<?> values = sr.getAttributes().get("member").getAll();
